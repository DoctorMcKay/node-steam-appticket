...
module.exports = parseEncryptedAppTicketEx;
 ...
 * @returns {object}
 */
function parseEncryptedAppTicketEx(ticket, encryptionKey) {
	try {
		...
		let ownershipTicket = parseAppTicket(decrypted.slice(outer.cb_encrypteduserdata, outer.cb_encrypteduserdata + ownershipTicketLength));
		if (ownershipTicket) {
